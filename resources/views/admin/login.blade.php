...
<div class="min-h-[70vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8 glass-panel p-8 sm:p-10 rounded-3xl relative overflow-hidden shadow-2xl">
        <div class="absolute inset-0 overflow-hidden pointer-events-none -z-10">
            <div class="absolute -right-10 -top-10 w-48 h-48 bg-amber-500/10 rounded-full blur-[40px]"></div>
        </div>

        <!-- Header -->
        <div class="text-center">
            <div class="mx-auto h-12 w-12 bg-amber-600 rounded-xl flex items-center justify-center shadow-lg shadow-amber-500/20 mb-4">
            ...
            <p class="mt-2 text-xs text-slate-400">
                Sign in to manage the bookstore catalog
            </p>
        </div>
        ...
        <form class="mt-8 space-y-6" action="{{ route('admin.login.submit') }}" method="POST">
            ...
            <div class="space-y-4 rounded-md">
                ...
                <div>
                    ...
                    <input id="email" name="email" type="email" autocomplete="email" required value="{{ old('email') }}" class="appearance-none relative block w-full px-4 py-3 border border-slate-800 bg-slate-900 placeholder-slate-500 text-white rounded-xl focus:outline-none focus:ring-1 focus:ring-amber-500 focus:border-amber-500 text-sm" placeholder="admin@bookstore.com">
                </div>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    <input id="remember" name="remember" type="checkbox" class="h-4 w-4 text-amber-600 focus:ring-amber-500 border-slate-800 rounded bg-slate-900">
            ...
            <!-- Submit -->
            <div>
                <button type="submit" class="group relative w-full flex justify-center py-3.5 px-4 border border-transparent text-sm font-bold rounded-xl text-white bg-amber-600 hover:bg-amber-500 shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500">
                    Admin Sign In
                </button>
            </div>
        </form>

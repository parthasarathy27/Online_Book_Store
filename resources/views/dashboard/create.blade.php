...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="glass-panel p-8 sm:p-10 rounded-3xl relative overflow-hidden shadow-2xl">
        ...
        <form action="{{ route('dashboard.books.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Image URL Fallback -->
                <div>
                    <label for="image_url_fallback" class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Or Cover Image URL</label>
                    <input type="url" name="image_url_fallback" id="image_url_fallback" value="{{ old('image_url_fallback', request('image_url')) }}" class="w-full px-4 py-3 border border-slate-800 bg-slate-900 text-white rounded-xl focus:outline-none focus:ring-1 focus:ring-brand-500 focus:border-brand-500 text-sm" placeholder="https://example.com/cover.jpg">
                    <p class="text-[10px] text-slate-500 mt-1">Used if no file is uploaded. Great for API imported links.</p>
                </div>
            </div>
